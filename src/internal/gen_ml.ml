...
let ident_of_ml_prim : type a. a Ctypes_primitive_types.ml_prim ->
  noalloc:bool -> Parsetree.core_type =
...
let rec typ_of_ctyp : type a. mod_path:string list -> a typ -> 'b =
  let open Ctypes_static in fun ~mod_path t ->
...
and typ_of_fn :
  type a. mod_path:string list -> a fn -> 'b =
...
let create_struct ~mod_path ~type_name ~field_names ~locs str =
  let rec iter ~mod_path names locs fields i params ac =
    ...
    | [],[],[] ->
      ...
      Type.mk ~params ~kind:(Ptype_record fields) n,
      List.map ~f:fst params
    | [],_,_ | _,[],_ | _,_,[] -> assert false
    | name::tl1, loc::tl2, (CS.BoxedField{CS.ftype;_})::tl3 ->
      U.with_loc loc @@ fun () ->
      let t,i,params = match typ_of_ctyp ~mod_path ftype with
      | `Incomplete,_ ->
        let s =
        ...
        t,succ i, (a::params)
      | `Complete, t -> t,i,params in
      let field = Type.field (U.mk_loc name) t in
      iter ~mod_path tl1 tl2 tl3 i params (field::ac) in
  let rec unbox : type a. a typ  -> 'b = function
  | CS.View { CS.ty ; _ } -> unbox ty
  | CS.Struct { CS.fields ; _ } ->
    (* twice. Once with referefence from within the top module, once from
       the bottom *)
...
let ml_typ_of_arg_typ ~mod_path ~noalloc t =
  let rec iter : type a. bool -> a typ  -> 'b =
    ...
    | Void -> `Complete, mk_typ "unit"
    | Primitive p ->
      if inside_view && noalloc = false then
        `Incomplete, Typ.any ()
      else
        `Complete, ident_of_ml_prim ~noalloc (Ctypes_primitive_types.ml_prim p)
    | Pointer x ->
    ...
    | Funptr fn    ->
      ...
      | `Complete,s -> `Complete, mk_typ ~l:[s] "Ctypes_static.static_funptr")
    ...
    | Array _    as a ->
      U.error "Unexpected array in an argument type: %s" (Ctypes.string_of_typ a)
    | Bigarray _ as a ->
      U.error
    ...
    | OCaml FloatArray ->
      ...
      `Complete, mk_typ ~l:[array] "Ctypes_static.ocaml" in
  iter false t

let rec ml_typ_of_return_typ :
  type a. a typ -> noalloc:bool -> inside_view:bool -> Parsetree.core_type =
  let open Ctypes_static in
  let mk_ptr noalloc =
...
let pat_expand_in t ~noalloc ~fparam ?type_expr param_name =
  let error t =
    ...
    U.error "cstubs does not support passing %s as parameter" x in
  let rec iter :
    type a. a typ -> fparam:Parsetree.expression
    -> ?type_expr:Parsetree.expression -> bool
  ...
  and inline_view : type a b. a typ -> b typ -> fparam:Parsetree.expression
    -> ?type_expr:Parsetree.expression -> bool
  ...
  fun tparent tchild ~fparam ?type_expr inside_view ->
    if Created_types.is_typedef_struct tparent then
      ...
      match pat_trans with
      | None,None -> Some pat_trans
      | _ -> None
      ...
      Some(None, Some e)
    else match inside_view, tchild, type_expr with
...
let pat_expand_out ?type_expr ~noalloc t param_name  =
  let error t =
    let x = Ctypes.string_of_typ t in
    U.error "cstubs does not support %s as return values" x in

  let rec iter : type a. ?type_expr: Parsetree.expression -> a typ -> bool
    ...
    fun ?type_expr t inside_view ->
      ...
      | Bigarray _ -> error t
  and inline_view :
    type a b. ?type_expr: Parsetree.expression -> a typ -> b typ -> bool
    -> (Parsetree.pattern option *
    ...
    fun ?type_expr tparent tchild inside_view ->
      if Created_types.is_typedef_struct tparent then
        ...
        let f expr =
          [%expr Ppx_cstubs_internals.string_read [%e expr]] in
        Some(None,Some f)
      else if tparent == Obj.magic Ctypes.string_opt then
        let f expr =
          [%expr Ppx_cstubs_internals.string_opt_read [%e expr]] in
        Some(None,Some f)
      else match inside_view, tchild, type_expr with
      | false, CS.Pointer _ , Some x ->
        (match extract_pointer_opt x with
        | Some orig_expr ->
          let f expr = [%expr
            ...
            | 0n -> None
            | ppxc__not_null ->
              Some (Cstubs_internals.make_ptr [%e orig_expr]
                      (Ppx_cstubs_internals.to_voidp ppxc__not_null))] in
          Some(None,Some f)
        | None -> None)
      | _ -> None in
...
let ctypes_typ_constr expr i ct =
  let expr = match ct.ptyp_desc with
  | Ptyp_constr ({txt = Ldot (Lident "Ctypes", "typ"); _ },
                 [{ptyp_desc = Ptyp_any;_}]) -> expr
  | _ -> Exp.constraint_ expr ct in
...
let build_ignore_expr (el,params) (retexpr,ret_info) =
  ...
  let i,e = ListLabels.fold_left2 ~init:(0,None) el params
      ~f:(fun (i,ac) (_,e) a ->
        ...
        (succ i),ac) in
  let e = f e i retexpr ret_info.rmatch_pat ret_info.rannot_needed
      ret_info.rconstr_ptype in
  match e with
  | None -> None
...
let build_parallel_pattern (el,param_infos) (retexpr,ret_info) func =
  ...
  | None -> l
  | Some s -> (ctypes_typ_constr retexpr !cnt ret_info.rconstr_ptype, s)::l in
  if l = [] then func
  else
  let dummy_end = {
    pc_lhs = Pat.any ();
...
let collect_info fn ~mod_path cinfo lexpr =
  ...
  let rec iter : type a. a Ctypes.fn -> Parsetree.expression list
    -> param_info list -> param_info list * ret_info =
    fun fn lexpr accu ->
      let type_expr,loc,lexpr' = match lexpr with
      | [] -> None, !Ast_helper.default_loc, []
      | hd::tl -> Some hd, hd.pexp_loc,tl in
      U.with_loc loc @@ fun () ->
      ...
      | CS.Returns t ->
        ...
        let rmatch_pat, res_trans =
          pat_expand_out ~noalloc ?type_expr t param_name in
        ...
        List.rev accu, {rext_ptype; rmatch_pat; rannot_needed; res_trans;
                        ris_inline_ocaml_type; rconstr_ptype}
      | CS.Function(a,b) ->
        let fun_expr,fun_pat = mk_ex_pat @@ param_name () in
        let match_pat,param_trans =
          pat_expand_in ?type_expr a ~fparam:fun_expr param_name ~noalloc in
        ...
        let i = {ext_ptype; fun_pat; fun_expr; annot_needed; constr_ptype;
                 match_pat; param_trans; label; is_inline_ocaml_type} in
        iter b lexpr' (i::accu) in
  iter fn lexpr []
...
let build_fun_constraint params ~return_errno =
  ...
  let start = match return_errno with
  | false -> vl
  | true -> Typ.tuple [vl; mk_typ "Signed.sint"] in
...
let common ~mod_path fn ext_name cinfo common =
  ...
  | `Extern(el,ret) ->
    ...
    ret_info
  | `Foreign_value _ | `Foreign _ -> collect_info ~mod_path fn cinfo [] in
  ...
  let build_stri already_constrained res =
    ...
    let no_type_repeat () =
      let res,fun_pat =
        if ret_info.rmatch_pat <> None ||
           List.exists param_infos ~f:(fun x -> x.match_pat <> None) then
          res_c,ext_fun_pat_c
        else
          res,ext_fun_pat in
      match common with
    ...
    | `Foreign fn_expr ->
      if ret_info.rannot_needed ||
         List.exists param_infos ~f:(fun s -> s.annot_needed) then
