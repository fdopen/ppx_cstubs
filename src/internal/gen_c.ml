...
type ret = {
  ...
  inj_byte_noalloc: (string -> string); (* caml_copy_double(call_native(..,.. *)
  ibyte_native_accessor_differ: bool;
  inative_ret_type:  string; (* if something else than value should be used,
                                necessary to support passing unboxed values *)
}

let ret_info_prim :
  type a. a Ctypes_primitive_types.prim -> ocaml_ret_var:string
  -> c_rvar:string -> decl_rvar:(unit -> string) -> ret  =
...
let rec ret_info : type a. a typ
  ...
  fun t ~user_noalloc ~ocaml_ret_var ~c_rvar ~decl_rvar ->
    let pptr () = {
      ...
      inj_alloc = (fun () ->
        Printf.sprintf
          "%s = CTYPES_FROM_PTR(%s);" ocaml_ret_var c_rvar);
      inj_noalloc = (fun () ->
        Printf.sprintf
          "%s = (intnat)(%s);" ocaml_ret_var c_rvar);
      inj_byte_noalloc = (Printf.sprintf "CTYPES_FROM_PTR(%s)");
      ibyte_native_accessor_differ = true;
      inative_ret_type = "intnat" } in
    ...
    let cp () =
      let inj () =
        Printf.sprintf
          "%s = ctypes_copy_bytes(&(%s),sizeof(%s));"
          ocaml_ret_var c_rvar c_rvar in
      standard ~inj_noalloc_possible:false inj in
    match t with
    | Void ->
...
type param = {
  ...
  native_param_type:  string; (* if something else than value should be used,
                                 necessary to support passing unboxed values *)
}


let pinfo_prim :
  type a. a Ctypes_primitive_types.prim
  -> ctype:string
  -> c_var:string
  -> ocaml_param:string
  -> param =
  ...
  fun p ~ctype ~c_var ~ocaml_param ->
    let f s =
      ...
      (fun g -> Printf.sprintf "%s(%s)" s g) in
    let prj_alloc, prj_byte_noalloc' = match p with
    | Char -> f "Int_val"
    | Schar -> f "Int_val"
    | Uchar -> f "Uint8_val"
    | Bool -> f "Bool_val"
    ...
    | Complexld -> f "ctypes_ldouble_complex_val" in

    let fprj_noalloc xtype =
      if xtype = ctype then
        fun () -> Printf.sprintf "%s %s = %s;" ctype c_var ocaml_param
      else
      fun () -> Printf.sprintf "%s %s = (%s)(%s);" ctype c_var ctype ocaml_param
    in
    let int_t t = {
      is_void = false;
      ocaml_param;
...
let gen_common a ~locs ~stubname ~cfunc_value ~release_runtime_lock ~noalloc
  ...
  let noalloc =
    ...
    (* [@unboxed] and [@untagged] only supported since 4.03.0.
       No workarounds in order to support older versions :D *) in
  let gen_byte_version =
    params_length > 5 ||
    (noalloc && (
       List.exists params ~f:(fun x -> x.byte_native_accessor_differ) ||
       ret.ibyte_native_accessor_differ)) in

  let buf = Buffer.create 128 in
  let cname_main = stubname in
  let cname_byte = "b" ^ stubname in (* first character is always significant.
...
|};
  ...
  let params_with_protection =
    ...
    else List.filter_map params ~f:(fun x ->
      if x.runtime_protect = false then None
      else Some x.ocaml_param) in
  ...
  if return_errno then
    Printf.bprintf buf "  errno = 0;\n";

  if ret.is_rvoid = false then
    Printf.bprintf buf "  %s = " ret.c_rvar
  else
    Buffer.add_string buf "  ";
  ...
  | `Fun cfunc ->
    ...
    List.iteri params ~f:(fun i x ->
      if i <> 0 then
        Buffer.add_string buf ", ";
      if x.is_void = false then
        Buffer.add_string buf x.c_var);
    Buffer.add_string buf ");\n";
  ...
  let return_val =
    if ret.is_rvoid then
      "Val_unit"
    else
      ret.ocaml_ret_var in
  let return_val =
    if return_errno = false then
      return_val
    else
      Printf.sprintf "ctypes_pair_with_errno(%s)" return_val in
  ...
  if gen_byte_version then (
    ...
    let l = List.mapi params ~f:(fun i x ->
      let t =
        if params_length < 6 then
          Printf.sprintf "a%d" i
        else
          Printf.sprintf "a[%d]" i in
      if noalloc then x.prj_byte_noalloc(t) else t) in
