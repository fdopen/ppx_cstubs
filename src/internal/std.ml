...
module Util = struct
  ...
  let mk_lid ?loc s =
    let s =
      ...
      | [] -> Lident ""
      | hd :: tl ->
        CCListLabels.fold_left ~f:(fun p s -> Ldot (p, s)) ~init:(Lident hd) tl
    in
    mk_lid_c ?loc s
  ...
  let ocaml_warning s =
    ...
    let pl = PStr [ [%stri [%e str_expr s]] ] in
  ...
  let alias_impl_mod_os ?(alias_name = !Lconst.impl_mod_name) () =
    ...
    A.Str.open_ (A.Opn.mk ~override:Override x)
  ...
  let alias_type ?attrs e =
    let res ?attrs e =
      match attrs with
      | None -> e
      | Some a -> { e with pexp_attributes = e.pexp_attributes @ a }
    in
