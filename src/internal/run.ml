...
let rec eintr2 f a b =
  try f a b with Unix.Unix_error (Unix.EINTR, _, _) -> eintr2 f a b

let rec eintr3 f a b c =
  try f a b c with Unix.Unix_error (Unix.EINTR, _, _) -> eintr3 f a b c

let rec eintr4 f a b c d =
  try f a b c d with Unix.Unix_error (Unix.EINTR, _, _) -> eintr4 f a b c d

let rec eintr6 f a b c d e g =
  try f a b c d e g
  with Unix.Unix_error (Unix.EINTR, _, _) -> eintr6 f a b c d e g
...
let run ?(env = Unix.environment ()) ?(stdin = `Null) ?(stderr = `Stderr)
    ?(stdout = `Stdout) prog args : int =
  ...
  let f_read r =
    ...
    if x <= 0 then
      if is_stdout then close_pipe p_stdout_read else close_pipe p_stderr_read
    else
  ...
  let ret_code =
    ...
    | Unix.WSTOPPED _ ->
      (* only possible if the call was done using WUNTRACED or when the child
         is being traced *)
      3
  in
  ret_code
