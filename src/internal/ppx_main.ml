...
module Extract : sig
  ...
  val type_decl : Asttypes.rec_flag -> type_declaration list -> ctyp list
end = struct
...
module H : sig
  val constant : (Asttypes.arg_label * expression) list -> expression

  val bound_constant :
    string -> (Asttypes.arg_label * expression) list -> expression

  val foreign_value : (Asttypes.arg_label * expression) list -> expression

  val header : (Asttypes.arg_label * expression) list -> expression

  val seal : (Asttypes.arg_label * expression) list -> expression
  ...
  val foreign :
    ?prefix:string -> (Asttypes.arg_label * expression) list -> expression
  ...
  val union : string -> (Asttypes.arg_label * expression) list -> structure_item

  val structure :
    string -> (Asttypes.arg_label * expression) list -> structure_item
    ...
    let cnt_structs, cnt_records =
      List.fold_left tl ~init:(0, 0) ~f:(fun ((cns, cnr) as ac) ->
        ...
        | Struct s ->
          ...
          (cns, cnr))
    in
    if cnt_structs > 1 && cnt_records >= 1 && type_rec_flag = Asttypes.Recursive
  ...
  let int_alias =
    ...
    fun ~mod_name alias_typ l ->
      ...
      let t = Typ.constr (U.mk_loc (Longident.Ldot (lid.txt, "t"))) [] in
...
module Scan () : sig
  ...
  val open_declaration : mapper -> open_declaration -> open_declaration
end = struct
  ...
  and module_type mapper m =
    ...
    | Pmty_typeof _ ->
      Static_level.disable @@ fun () ->
      Save_position.disable @@ fun () -> default_mapper.module_type mapper m
    | Pmty_ident _ | Pmty_signature _ | Pmty_functor _ | Pmty_with _
    | Pmty_extension _ | Pmty_alias _ ->
      default_mapper.module_type mapper m

  and payload mapper p =
    Static_level.disable @@ fun () ->
    Save_position.disable @@ fun () -> default_mapper.payload mapper p

  and open_declaration mapper p =
    Open_decl.new' p @@ fun () -> default_mapper.open_declaration mapper p
end

module Replace : sig
  val structure : mapper -> structure_item list -> structure_item list
  ...
  let structure mapper str =
    ...
    |> List.rev_map ~f:(function
         | {
             pstr_desc = Pstr_open ({ popen_attributes = _ :: _ as l; _ } as d);
             _;
           } as w ->
           let l =
...
let mapper top _config _cookies =
  ...
  let open P in
  let module Scan = Scan () in
  let phase = ref Initial_scan in
  ...
  let structure mapper str =
    ...
    | Initial_scan ->
      if !is_outer_structure = false || str = [] then
        ...
        phase := Replace;
        let st = Replace.structure mapper st in
  ...
  let structure_item mapper stri =
    ...
    match !phase with
    | Initial_scan -> Scan.structure_item mapper stri
    | Replace -> Replace.structure_item mapper stri
  in
  let expr mapper pexp =
    Ast_helper.default_loc := pexp.Parsetree.pexp_loc;
    match !phase with
    | Initial_scan -> Scan.expr mapper pexp
    | Replace -> Replace.expr mapper pexp
  in
  let module_expr mapper m =
    Ast_helper.default_loc := m.pmod_loc;
    match !phase with
    | Initial_scan -> Scan.module_expr mapper m
    | Replace -> default_mapper.module_expr mapper m
  in
  let module_type mapper m =
    Ast_helper.default_loc := m.pmty_loc;
    match !phase with
    | Initial_scan -> Scan.module_type mapper m
    | Replace -> default_mapper.module_type mapper m
  in
  let payload mapper p =
    match !phase with
    | Initial_scan -> Scan.payload mapper p
    | Replace -> default_mapper.payload mapper p
  in
  let open_declaration mapper o =
    Ast_helper.default_loc := o.popen_loc;
    match !phase with
    | Initial_scan -> Scan.open_declaration mapper o
    | Replace -> default_mapper.open_declaration mapper o
  in
  let type_declaration mapper t =
    Ast_helper.default_loc := t.ptype_loc;
    match !phase with
    | Initial_scan -> default_mapper.type_declaration mapper t
    | Replace -> Replace.type_declaration mapper t
  in
  {
    default_mapper with
    structure_item;
