...
let prepare : type a. a Ctypes_static.typ -> info option =
  ...
  let module C = Ctypes_static in function
  | C.Void -> None
  | C.Struct _ -> None
  | C.Union _ -> None
  | C.Array _ -> None
...
let gen t str =
  ...
  let camlint_max =
    if Ocaml_config.word_size () = 64 then
      ...
      Big_int.big_int_of_int 1073741823 in
  let camlint_min =
    if Ocaml_config.word_size () = 64 then
      Big_int.big_int_of_int64 (-4611686018427387904L)
    else
      Big_int.big_int_of_int (-1073741824) in
  let intnative_max =
    if Ocaml_config.word_size () = 64 then
      Big_int.big_int_of_int64 9223372036854775807L
    else
      Big_int.big_int_of_int32 2147483647l in
  let intnative_min =
    if Ocaml_config.word_size () = 64 then
      Big_int.big_int_of_int64 (-9223372036854775808L)
    else
      Big_int.big_int_of_int32 (-2147483648l) in
  ...
  let rec as_sum ~add ~of_int ~of_int64 r =
    if r <= camlint_max then
      [%expr [%e of_int] [%e xint r]]
    else if r <= int64_max then
      [%expr [%e of_int64] [%e xint64 r]]
    else
    let (-) = Big_int.sub_big_int in
    let e = as_sum ~add ~of_int ~of_int64 (r - int64_max) in
    [%expr [%e add] [%e e] ([%e of_int64] [%e xint64 int64_max])] in

  let r = Big_int.big_int_of_string str in
  match t.info with
  | Extr_char ->
    Char.chr (((Big_int.int_of_big_int r) + 256 ) mod 256)
    |> X.char
  ...
  | Extr_sint ->
    if r >= camlint_min && r <= camlint_max then
      ...
      [%expr Signed.SInt.of_string [%e xstr r]]
  | Extr_long ->
    if r >= camlint_min && r <= camlint_max then
      ...
      [%expr Signed.Long.of_string [%e xstr r]]
  | Extr_llong ->
    if r >= camlint_min && r <= camlint_max then
  ...
  | Extr_uint ->
    if r <= camlint_max then
      ...
      [%expr Unsigned.UInt.of_int64 [%e xint64 r]]
    else
      [%expr Unsigned.UInt.of_string [%e xstr r]]
  | Extr_ulong ->
    as_sum r
      ~add:[%expr Unsigned.ULong.add]
      ~of_int:[%expr Unsigned.ULong.of_int]
      ~of_int64:[%expr Unsigned.ULong.of_int64]
  | Extr_ullong ->
    as_sum r
      ~add:[%expr Unsigned.ULLong.add]
      ~of_int:[%expr Unsigned.ULLong.of_int]
      ~of_int64:[%expr Unsigned.ULLong.of_int64]
  | Extr_size_t ->
    as_sum r
      ~add:[%expr Unsigned.Size_t.add]
