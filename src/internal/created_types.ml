...
let t_predefined =
  [
    ...
    View_predefined {vp_ct = fun () ->
      U.mk_typc ~l:[U.mk_typc "string"] "option"};
...
let init_predef () =
  List.iter t_predefined ~f:(fun s ->
    CCVector.push v_types s)
...
let add_type :  'a Ctypes.typ -> t -> unit = fun t s ->
  (* TODO: optimize for faster find. But the order must be preserved *)
  let t = abstract t in
  CCVector.push v_types (t,s)

let find : 'a Ctypes.typ -> t option = fun t ->
  let t = abstract t in
  CCVector.find_map (fun (t',info) ->
    if t' == t then Some info else None) v_types
...
| View_structured { vs_state = Vs_Unknown; _ }
| Custom _ -> `Unknown
...
| Structured {s_type_id; s_is_union; _ } ->
  ...
  let s = match s_is_union with
  | false -> "Ctypes_static.structure"
  | true -> "Ctypes_static.union" in
...
let get_core_type t mod_path =
  match find t with
  | None -> `Unknown
  | Some s -> get_core_type mod_path s
...
let change_struct_view_state t s =
  ...
  | Some t ->
    ...
    | Custom _  | View_typedef_structured _ | View_enum _
    | View_predefined _ | Structured _ -> assert false
...
let is_typedef_struct t =
  ...
  | Some t ->
    ...
    | View_structured _ | Custom _   | View_enum _
    | View_predefined _ | Structured _ -> false
