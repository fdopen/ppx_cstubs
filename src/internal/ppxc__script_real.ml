...
#include <ctypes_cstubs_internals.h>
  ...
  let get_extract_source () =
    List.fold_left ~init:[] !entries ~f:(fun a -> function
      | Header h | Extract_text h -> h :: a | Replace_text _ -> a )
    |> String.concat "\n"
...
module Const = struct
  ...
  let add ?disable_checks ~no_expr id loc t str ~info_str =
    ...
    let rinfo =
      ...
      | `String ->
        Extract_c.prepare_extract_string ~buf:extract_source ~c_header
          ~expr:str ()
    in
    ...
    |> List.iter ~f:(fun (_id, (loc, _einfo, rinfo, info_str, _no_expr)) ->
           ...
           | Error _ ->
             if Buffer.length ebuf > 0 then
               if !Options.verbosity > 0 then Buffer.output_buffer stderr ebuf
               else
          ...
          let obj =
            ...
            | None -> (
              ...
              | Error s -> (
                ...
                | Some (loc, s') ->
                  U.error ~loc "%s ; couldn't extract %s" s s'
                | None -> U.error "%s" s )
              | Ok o ->
                obj_code := Some o ;
                o )
  ...
  let extract_all () = Hashtbl.iter extract_common htl_id_entries

  let extract_single id =
    let x =
      match Hashtbl.find htl_id_entries id with
      | exception Not_found ->
        U.error "fatal: constant not found in hash table"
      | x -> x
    in
    extract_common id x
...
module Extract = struct
  ...
  let field id_loc stru field_name field_type =
    ...
    let dummy_header =
      Printf.sprintf "static %s;\n static %s;\n" var1_printed var2_printed
    in
    C_content.add_extract_source dummy_header ;
    let to_extract =
      Printf.sprintf
        "((offsetof(%s,%s))|(PPXC_TYPES_COMPATIBLE(%s.%s,%s) << 29))" ctyp
  ...
  let seal id_loc_size id_loc_align stru =
    ...
    Const.add ~info_str:("size of " ^ ctyp) ~no_expr:false ~disable_checks:true
  ...
  let enum id_loc ?(typedef = false) name =
    ...
    Const.add ~no_expr:true ~info_str ~disable_checks:true id loc
      Ctypes.camlint str ;
    (* wrong Ctypes.typ, wrong arithmetic type - but it's not used here to
       build externals *)
    Cstubs_internals.build_enum_type name Ctypes_static.Int32 ~typedef
      [(`A, 1L)]
  ...
  let enum2 (l : 'a list) (p : string) : 'a Ctypes.typ * 'a list Ctypes.typ =
    ...
    List.iter enum_l ~f:(fun c ->
        ...
        Const.add ~no_expr:true ~info_str c.ee_signed_id c.ee_loc
          Ctypes.int64_t c.ee_cname ;
        Const.add ~no_expr:true ~info_str c.ee_unsigned_id c.ee_loc
          Ctypes.uint64_t c.ee_cname ) ;
...
module Build = struct
  ...
  let constant =
    ...
    fun id_loc str ->
      ...
      let res =
        match Hashtbl.find Const.htl_id_str_result id with
        | x -> x
        | exception Not_found -> U.error "couldn't extract constant %S" str
      in
      let ef t =
  ...
  let enum id_loc name ?(typedef = false) ?unexpected l =
    let id, _loc = U.from_id_loc_param id_loc in
    let a_type, a_type_expr =
      match enum_size_unsigned id name with
      ...
      | x, _ -> U.error "enum %s has unsupported size of %d" name x
    in
  ...
  let enum2 (l : 'a list) (p : string) : 'a Ctypes.typ * 'a list Ctypes.typ =
    ...
    let list =
      ListLabels.fold_right ~init:[%expr []] enum_l ~f:(fun c ac ->
          ...
          @@ fun () ->
          let id =
            match type_unsigned with
            | true -> c.ee_unsigned_id
            | false -> c.ee_signed_id
          in
          Const.extract_single id ;
          let i_str =
  ...
  let foreign id_loc_external id_loc_stri_expr mp ~ocaml_name ~typ_expr
      ?(release_runtime_lock = false) ?(noalloc = false)
      ?(return_errno = false) c_name fn =
    let id_external, _loc_external = U.from_id_loc_param id_loc_external in
    let id_stri_expr, _loc_stri_expr = U.from_id_loc_param id_loc_stri_expr in
    let mod_path : string list = Marshal.from_string mp 0 in
    let stubname = U.safe_cname ~prefix:c_name in
  ...
  let create_record id params t =
    let open Marshal_types in
    let { sr_type_name = type_name
        ; sr_locs = locs
        ; sr_field_names = field_names
        ; sr_mod_path = mod_path } =
      Marshal.from_string params 0
    in
    let ((_, _, params) as res) =
      Gen_ml.create_struct ~mod_path ~type_name ~field_names ~locs t
    in
    let state =
      match params with
      | [] -> Created_types.Vs_Complete
      | _ :: _ -> Created_types.Vs_Parameterized
    in
  ...
  let field id_loc stru field_name _field_type =
    ...
    Const.extract_single id ;
    let r =
      match Hashtbl.find Const.htl_id_str_result id |> int_of_string with
      | x -> x
...
module Run_environment = struct
  ...
  let funptr ?abi ?name ?check_errno ?runtime_lock ?thread_registration fn =
    G.foreign_used := true ;
    Foreign.funptr ?abi ?name ?check_errno ?runtime_lock ?thread_registration
      fn
