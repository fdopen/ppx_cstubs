...
let attr ~attr ~cont =
  [U.mk_loc attr,PStr [[%stri [%e U.str_expr cont]]]]

let vb ~attrs n expr =
  Str.value Nonrecursive [Vb.mk ~attrs (U.mk_pat n) expr]
...
let make mod_path short_name expr =
  ...
  let attrs =
    (U.mk_loc "ocaml.warning", PStr [[%stri "-32"]])::
    (attr ~cont:sref ~attr:a_inmod_ref) in
  let topmod_ref =
    vb ~attrs short_name (U.mk_ident uniq_name) in
...
let get_remove_string_exn name attr =
  let res = ref None in
  let attribs = List.filter attr ~f:(fun (x,t) ->
    ...
    | PStr [{pstr_desc = Pstr_eval(
      ...
      res := Some s;
      false
    | _ -> failwith "surprising content in attribute") in
  match !res with
...
let is_uniq htl orig =
  ...
  | [_] ->
    (* TODO: Better representation than string *)
    let module M = struct exception F end in
    let pre,suf = match CCString.Split.right ~by:"." orig with
    | Some(a,b) -> a ^ ".", "." ^ b
    | None -> "","." ^ orig in
    (try
      Hashtbl.iter (fun sref () ->
        if sref <> orig &&
           CCString.prefix ~pre sref &&
           CCString.suffix ~suf sref
        then
          raise_notrace M.F) htl;
      true
    with
    | M.F -> false)
...
| { pexp_desc = ((Pexp_ident _) as orig);
    pexp_attributes = ( (_::_) as attribs) ; _ } as expr
...
| {pstr_desc = Pstr_value(Nonrecursive,[
  ...
  if List.exists attribs ~f:(fun (x,_) -> x.txt == a_orig_name) then
    ...
    let s,pvb_attributes =
      get_remove_string_exn a_inmod_ref attribs in
    if is_uniq s then
      U.empty_stri ()
    else
...
let rec get_parents ~ref_p ~cur_p =
  ...
  | _::_,[] -> ref_p
  | hd1::tl1, hd2::tl2 ->
    if CCString.equal hd1 hd2 then get_parents ~ref_p:tl1 ~cur_p:tl2
    else ref_p
...
let make_type_alias ?tdl_attrs ?params mod_path short_name =
  ...
  let attrs = match tdl_attrs with
  | None -> attrs
  | Some x -> x@attrs in
  let manifest =
    let l = match params with
    | None -> []
    | Some s -> List.map ~f:fst s in
    U.mk_typc ~l short_name in
...
let create_type_ref_final ?(l=[]) id mod_path =
  ...
  let name = match is_uniq_type id.sref = false && paths = [] with
  | true -> id.uniq_name
  | false -> id.short_name in
...
| {ptyp_desc = (Ptyp_constr(_,lorig) as orig);
  ...
  let id_ref,single_ref = CCString.Split.left_exn ~by:"|" s in
  let ptyp_desc = (* types are uniq inside a module *)
